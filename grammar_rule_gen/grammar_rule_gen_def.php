...
$rule1 = new \Cheope_ppp_ns\src\Lex_rule(LEX_RULE_1);
$rule2 = new \Cheope_ppp_ns\src\Lex_rule(LEX_RULE_2);
$rule3 = new \Cheope_ppp_ns\src\Lex_rule(LEX_RULE_3);
$rule4 = new \Cheope_ppp_ns\src\Lex_rule(LEX_RULE_4);
$rule5 = new \Cheope_ppp_ns\src\Lex_rule(LEX_RULE_5);
//$dfa6 = &new Lex_dfa(LEX_RULE_6);

$rule1->setRegexp("/^[A-Z][A-Z_a-z0-9]*/");
$rule1->setTokenType(\Cheope_ppp_ns\src\Token::TYPE_LEXICAL_ELEMENT);
$rule1->setTokenVal(TOKEN_VAL_NONTERMINALE);

$rule2->setRegexp("/^[a-z][A-Z_a-z0-9]*/");
$rule2->setTokenType(\Cheope_ppp_ns\src\Token::TYPE_LEXICAL_ELEMENT);
$rule2->setTokenVal(TOKEN_VAL_TERMINALE);

$rule3->setRegexp("/^[\s]+/");
$rule3->setTokenType(\Cheope_ppp_ns\src\Token::TYPE_DELIM);
$rule3->setTokenVal(TOKEN_VAL_WS);

$rule4->setRegexp("/^\|/");
$rule4->setTokenType(\Cheope_ppp_ns\src\Token::TYPE_SPECIAL_ITEM);
$rule4->setTokenVal(TOKEN_VAL_OR);

$rule5->setRegexp("/^=/");
$rule5->setTokenType(\Cheope_ppp_ns\src\Token::TYPE_SPECIAL_ITEM);
$rule5->setTokenVal(TOKEN_VAL_EQUAL);

$rules = new \Cheope_ppp_ns\src\Lex_rules_container(LEX_RULE_CONTAINER_1);
$rules->add($rule1);
$rules->add($rule2);
$rules->add($rule3);
$rules->add($rule4);
...
$grRules = new \Cheope_ppp_ns\src\Parser_grammar_rules_container(PARSER_GRAMMAR_RULE_CONTAINER_1);
$grRules->add($grRule1);
?>